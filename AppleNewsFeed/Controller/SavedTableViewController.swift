...
class SavedTableViewController: UITableViewController {
    ...
    func saveCoreData() {
        do {
            try context?.save()
            basicAlert(title: "Deleted!", message: "Article is successfully deleted from Core Data.")
        }catch{
            print(error)
        }
        loadCoreData()
    ...
    override func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        ...
        if savedItems.count == 0 {
            tableView.setEmptyView(title: "You don't have any saved articles.", message: "Go to Apple News Tab Bar to check the articles.")
        }
        else {
            tableView.restore()
        }
